...
(function (root, name, make) {
     ...
     * @param {{length:number}} target receiver (becomes array-like if not already)
     * @param {*=} item or stack to push into `stack`
     */
    ...
    /**
    * $()
    * @param {?(string|Node|Object|{length:number}|Function|*)=} item
    * @param {?(Node|{length:number})=} root node(s) from which to base selector queries
    * @return {Dj}
    */  
    ...
    // Sync the prototypes
    // http://github.com/ender-js/ender-js/pull/17
     ...
     * @link http://github.com/ryanve/blood
     * @link http://stackoverflow.com/q/16594717/770127
     * @return {Object}
     */
    function resample() {
     ...
     *   false => receiver[n] must be null|undefined, supplier[n] must NOT be null|undefined
    ...
    /**
     * Integrate applicable methods|objects into a host. Other 
     ...
     * @link https://github.com/ryanve/submix
     ...
     *   For default behavior `$` should be omitted or set to `undefined`. This param 
     *   allows you to bridge to a receiver, but relay methods based on a another host,
     *   for example `someModule.bridge({}, false, jQuery)`. Set `$` explicity to `null`
     *    *only* if you want to communicate to relays that there should be *no* main api.                                   
     */
        ...
        for (k in s) {
            ...
            if (typeof v == 'function' || typeof v == 'object' && v) {
                if ('fn' === k && v !== s) {
                    ...
                    if (relay !== false) {
                        // The check above provides a way to bypass non-agnostic props.
